...
def save_decoded_image(img, name):
    img = img.view(img.size(0), 3, 236, 236)
    save_image(img, name)
...
#Epochs
n_epochs = 40 #пока самый нормальный вариант, так как нет переобучения и фотографии получаются на выходе более чёткими
...
for epoch in range(1, n_epochs+1):
    ...
    #Training
    # for data_normal in train_normal_loader:
    ...
    #     loss = criterion(outputs, noisy_img)
    #     loss.backward()
    #     optimizer.step()
    #     train_loss += loss.item()
    
    # train_loss = train_loss/len(train_normal_loader)
    ...
    save_decoded_image(gt_images.cpu().data, name='./Saved_Images_3/noisy{}.png'.format(epoch))
    save_decoded_image(outputs.cpu().data, name='./Saved_Images_3/denoised{}.png'.format(epoch))
...
plt.figure()
plt.imshow(img)
save_decoded_image(outputs.cpu().data, name='./Saved_Images_collect/denoised.png')
